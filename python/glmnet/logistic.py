...
class LogitNet(BaseEstimator):
    ...
    def __init__(self, alpha=1, n_lambda=100, min_lambda_ratio=1e-4, lambda_path=None, cut_point=1.0,
                 standardize=True, fit_intercept=True, lower_limits=-np.inf, upper_limits=np.inf,
                 n_splits=3, scoring=None, n_jobs=1, tol=1e-7, max_iter=100000,
                  shuffle=True, random_state=None, max_features=None, verbose=False):
    ...
    def fit(self, X, y, sample_weight=None, relative_penalties=None, groups=None):
        ...
        if self.n_splits >= 3:
            if groups is None:
                self._cv = StratifiedKFold(n_splits=self.n_splits, shuffle=self.shuffle, random_state=self.random_state) 
            else:
            ...
            self.scores_lambda_best_ = self.cv_mean_score_[self.lambda_best_inx_] 
