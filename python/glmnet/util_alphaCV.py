...
def _score_path(est, X, y, groups, sample_weight, relative_penalties, scoring, n_jobs, verbose): 
    ...
    Returns
    -------
    scores : array, shape (n_alpha, n_lambda) 
        Scores for each combination of alpha and lambda over all cv folds.
    """
    
    scorer = check_scoring(est, scoring) 
    cv_split = est._cv.split(X, y, groups) 
    
    # We score the model for every value of alpha and lambda, for classification
    ...
    with warnings.catch_warnings():
        ...
        scores = Parallel(n_jobs=n_jobs, verbose=verbose, backend='loky')(
            ...
            delayed(_fit_and_score)(est, scorer, X, y, sample_weight, relative_penalties, 
                                    est.alpha_path_, est.lambda_path_, train_idx, test_idx) 
            for (train_idx, test_idx) in cv_split )
    
    return scores 

def _fit_and_score(est, scorer, X, y, sample_weight, relative_penalties,
                   alpha_path, lambda_path, train_inx, test_inx):
    ...
    alpha_path : array, shape (n_alpha,)
        The lambda values to evaluate/score.
    ...
    -------
    scores : array, shape (n_alpha, n_lambda)
        Scores for each value of alpha and lambda for a single cv fold.
    """
    
    n_alpha = len(alpha_path) 
    n_lambda = len(lambda_path) 
    
    scores = np.empty( (n_alpha, n_lambda ) )
    
    for i_alpha in range(n_alpha):
        m = clone(est) 
        # for fixed alpha in alpha_path 
        m.alpha = alpha_path[i_alpha] 
        # fit model on training data for each value of lambda in lambda_path 
        m = m._fit(X[train_inx, :], y[train_inx], sample_weight[train_inx], relative_penalties) 
        lamb = np.clip(lambda_path, m.lambda_path_[-1], m.lambda_path_[0]) 
        # return score along the lamda_path for our given alpha 
        scores[i_alpha] = scorer(m, X[test_inx, :], y[test_inx], lamb=lamb) 
