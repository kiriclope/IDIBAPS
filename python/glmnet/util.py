...
def _score_lambda_path_off_diag(est, X_t_train, X_t_test, y, groups, sample_weight, relative_penalties,
                       scoring, n_jobs, verbose):
    """Score each model found by glmnet using cross validation for off diagonal elements in temporal generalization procedure.
    ...
    """
    scorer = check_scoring(est, scoring)
    cv_split = est._cv.split(X_t_train, y, groups) 
    ...
    with warnings.catch_warnings(): 
        ...
        scores = Parallel(n_jobs=n_jobs, verbose=verbose, backend='loky')(delayed(_fit_and_score_off_diag)
                                                                          (est, scorer, X_t_train, X_t_test, y,
                                                                           sample_weight, relative_penalties, est.lambda_path_,
                                                                           train_idx, test_idx)
                                                                          for (train_idx, test_idx) in cv_split)
        
    scores = np.moveaxis(np.array(scores), 0, 1)
    return scores 

def _fit_and_score_off_diag(est, scorer, X_t_train, X_t_test, y, sample_weight, relative_penalties,
                            score_lambda_path, train_inx, test_inx):
    """Fit and score a single model for off diagonal elements in temporal generalization procedure.
    ...
    X_t_train : array, shape (n_samples, n_features)
        Input train features
    
    X_t_test : array, shape (n_samples, n_features)
        Input test features
    ...
    X_train = X_t_train[train_inx] 
    X_train_test = X_t_train[test_inx] 
    X_test = X_t_test[test_inx] 
    ...
    m = m._fit(X_train, y[train_inx], sample_weight[train_inx], relative_penalties)
    
    lamb = np.clip(score_lambda_path, m.lambda_path_[-1], m.lambda_path_[0])
        
    t_train_scores = scorer(m, X_train_test, y[test_inx], lamb=lamb) 
    t_test_scores = scorer(m, X_test, y[test_inx], lamb=lamb) 
    
    scores = np.vstack((t_train_scores, t_test_scores))
    return scores
