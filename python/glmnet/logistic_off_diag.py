...
from glmnet.util import (_fix_lambda_path,
                         _check_user_lambda,
                         _interpolate_model,
                         _score_lambda_path_off_diag)

class LogitNetOffDiag(BaseEstimator): 
    ...
    def fit(self, X_t_train, X_t_test, y, sample_weight=None, relative_penalties=None, groups=None):
        ...
        self.lambda_path = self.lambda_path # / X.shape[1] # to be consistent with sklearn 
        ...
        if self.n_splits >= 3:
            ...
            cv_scores = _score_lambda_path_off_diag(self, X_t_train, X_t_test, y, groups,
                                                                          sample_weight, relative_penalties,
                                                                          self.scoring, n_jobs=self.n_jobs,
                                                                          verbose=self.verbose)
            
            cv_train_scores = cv_scores[0] 
            cv_test_scores = cv_scores[1] 
            
            # # compute best lambda from t train scores 
            self.cv_mean_train_score_ = np.atleast_1d(np.mean(cv_train_scores, axis=0)) 
            self.cv_train_standard_error_ = np.atleast_1d(stats.sem(cv_train_scores)) 
            
            self.lambda_max_inx_ = np.argmax(self.cv_mean_train_score_) 
            self.lambda_max_ = self.lambda_path_[self.lambda_max_inx_] 
            
            target_train_score = self.cv_mean_train_score_[self.lambda_max_inx_] -\
                                 self.cut_point * self.cv_train_standard_error_[self.lambda_max_inx_]
                        
            self.lambda_best_inx_ = np.argwhere(self.cv_mean_train_score_ >= target_train_score)[0]
            self.fixed_lambda_inx = self.lambda_best_inx_
                
            self.lambda_best_ = self.lambda_path_[self.lambda_best_inx_] 
            
            # print('lambda_max', self.lambda_max_ , 'lambda_best', self.lambda_best_)
            # compute best lambda from t test scores 
            # self.cv_mean_test_score_ = np.atleast_1d(np.mean(cv_test_scores, axis=0))
            # self.cv_test_standard_error_ = np.atleast_1d(stats.sem(cv_test_scores)) 
            
            # self.lambda_max_inx_ = np.argmax(self.cv_mean_test_score_)
            # self.lambda_max_ = self.lambda_path_[self.lambda_max_inx_]
            
            # target_test_score = self.cv_mean_test_score_[self.lambda_max_inx_] -\
            #                      self.cut_point * self.cv_test_standard_error_[self.lambda_max_inx_]
            
            # self.lambda_best_inx_ = min( np.argwhere(self.cv_mean_test_score_ >= target_test_score) )
            # self.lambda_best_ = self.lambda_path_[self.lambda_best_inx_] 
            
            # # return best test score 
            self.cv_mean_test_score_ = np.atleast_1d(np.mean(cv_test_scores, axis=0)) 
            self.cv_mean_score_best_ = self.cv_mean_test_score_[self.lambda_best_inx_] 
