...
def bayesian_bootstrap_regression(X, y, statistic, n_replications, resample_size,low_mem=False, n_jobs=1):
    ...
    Parameter low_mem(bool): Use looping instead of generating all the Generator.dirichlet, use if program use too much memory
    ...
    def loop_par(X_arr, y_arr, w, resample_size):
        resample_i = np.random.choice(range(len(X_arr)), p=w, size=resample_size)
        resample_X = X_arr[resample_i]
        resample_y = y_arr[resample_i]
        s = statistic(resample_X, resample_y)
    ...
    with tqdm_joblib(tqdm(total=len(weights), desc='bayes bootstrap') ) as progress_bar: 
        samples = Parallel(n_jobs=n_jobs, verbose=False)(delayed(loop_par)(X_arr, y_arr, w, resample_size) for w in weights )
...
class BayesianBootstrapBagging(object):
    ...
    def __init__(self, base_learner, n_replications, resample_size, low_mem=False, n_jobs=1):
    ...
    def fit(self, X, y):
        ...
        self.base_models_ = bayesian_bootstrap_regression(X,
                                                          ...
                                                          self.n_replications,
                                                          self.resample_size,
                                                          low_mem=self.memo, n_jobs=self.n_jobs)
        return self
